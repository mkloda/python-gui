...
class Application(tk.Frame):
    def __init__(self, master):
        ...
        self.master = master
        master.title('Sklep z czesciami')
        master.geometry("600x330")
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        self.part_label = tk.Label(
            self.master, text='Nazwa czesci', font=('bold', 14), pady=20)
        ...
        self.customer_label = tk.Label(
            self.master, text='Kupujacy', font=('bold', 14))
        ...
        self.retailer_label = tk.Label(
            self.master, text='Producent', font=('bold', 14))
        ...
        self.price_label = tk.Label(
            self.master, text='Cena', font=('bold', 14))
        ...
        self.add_btn = tk.Button(
            self.master, text="Podaj czesc", width=12, command=self.add_item)
        self.add_btn.grid(row=2, column=0, pady=20)

        self.remove_btn = tk.Button(
            self.master, text="Usun czesc", width=12, command=self.remove_item)
        self.remove_btn.grid(row=2, column=1)

        self.update_btn = tk.Button(
            self.master, text="Zaktualizuj", width=12, command=self.update_item)
        self.update_btn.grid(row=2, column=2)

        self.exit_btn = tk.Button(
            self.master, text="Wyczysc", width=12, command=self.clear_text)
    ...
    def add_item(self):
        if self.part_text.get() == '' or self.customer_text.get() == '' or self.retailer_text.get() == '' or self.price_text.get() == '':
            messagebox.showerror(
                "Wymagane pole", "Prosze uzupelnic wszystkie pola")
            return
